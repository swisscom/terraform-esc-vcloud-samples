...
# VCD Native VM for db server
resource "vcd_vm" "aat-101-db-01" {
  name 						    = "db-server-01" // VM name
  computer_name       = "db-server-01" // Host name
...
resource "vcd_vm_internal_disk" "aat-101-db-01-disk1" {
  vapp_name  = vcd_vm.aat-101-db-01.vapp_name
  vm_name    = vcd_vm.aat-101-db-01.name
  bus_type = "paravirtual"
  size_in_mb = "10240"
  bus_number = 0
  unit_number = 1
  storage_profile = var.vcd_storage_policy
  depends_on      = [vcd_vm.aat-101-db-01]
}