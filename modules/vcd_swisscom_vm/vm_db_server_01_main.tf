...
data "template_file" "db_server_01_json" {
  ...
  vars = {
    ...
    network = "aat-101-db-servers-network"
  }
}

// VCD Custom (Swisscom) VM - VCD runtime defined entity 
resource "vcd_rde" "db_server_01_rde" {
  org          = var.vcd_org
  name         = "swisscom-vm-01"
  rde_type_id  = data.vcd_rde_type.swisscom_rde_type.id
  resolve      = false
  input_entity = data.template_file.db_server_01_json.rendered
...
output "db_server_01_rde_id" {
  value = vcd_rde.db_server_01_rde.id
}
...
resource "null_resource" "wait_for_db_server_01" {
  provisioner "local-exec" {
    ...
    if [ $? -eq 0 ]; then
      ...
      for i in $(seq 1 120); do
        ...
        swisscom_vm_rde_response=$(curl -s --request GET 'https://${var.vcd_api_host}/cloudapi/1.0.0/entities/${vcd_rde.db_server_01_rde.id}' \
        ...
        if [ $(echo "$swisscom_vm_rde_response" | jq -r '.state') = "RESOLVED" ]; then
            ...
            echo "$swisscom_vm_name" > "file_db_server_01_vm_name.txt"
  ...
  depends_on = [vcd_rde.db_server_01_rde]
}

// Read the value (generated vm name) from the file
data "local_file" "db_server_01_vm_name_file" {
  filename   = "file_db_server_01_vm_name.txt"
  depends_on = [null_resource.wait_for_db_server_01]
}
...
#============================
data "vcd_resource_list" "import_db_server_01_vm" {
  resource_type    = "vcd_vm"
  name             =  "${chomp(data.local_file.db_server_01_vm_name_file.content)}"
  vdc = var.vcd_vdc
  name_regex = "(${chomp(data.local_file.db_server_01_vm_name_file.content)})"
  list_mode        = "import"
  import_file_name = "vm_db_server_01_import.tf"
}