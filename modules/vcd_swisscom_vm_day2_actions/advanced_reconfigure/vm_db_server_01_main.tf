...
data "vcd_rde" "db_server_01_rde" {
  rde_type_id = data.vcd_rde_type.rde_type.id
  name        = "AAT10110275"
}

// Terraform data source for vcd rde
data "vcd_rde_behavior_invocation" "invoke_advance_reconfigure_db_01" {
  rde_id         = data.vcd_rde.db_server_01_rde.id
  behavior_id    = var.vcd_adv_reconfigure_rde_behavior_id
  arguments_json = data.template_file.db_server_01_adv_reconfigure_json.rendered
